...
def promote_seasonality(staging_df, baseline_df, baseline_30d_df, hourly_df, mode, tenant_anchor):
    ...
    for group_key, group in grouped:
        ...
        # tenant_anchor, specifically to catch a tenant whose own clock has stalled.
        if (datetime.utcnow() - last_seen).days > CONFIG["TTL_DAYS"][pattern_type]:
            continue
...
if __name__ == "__main__":
    ...
    daily_df = promote_seasonality(staging_df, baseline_df, baseline_30d_df, hourly_df, mode="daily_candidate", tenant_anchor=tenant_anchor)

    logger.info("\n" + "=" * 80)
    logger.info("RESULTS")
    logger.info("=" * 80)

    print(f"\n[OK] Daily patterns detected: {len(daily_df)}")
    ...
    if len(daily_df) > 0:
        ...
        try:
            ...
            logger.info(f"[OK] Verified: Total daily patterns in table: {total_count}")

            client.close()
            logger.info("[OK] Connection closed")

            print(f"\n[OK] SUCCESS: {len(daily_df)} daily patterns written to {TARGET_TABLE}")
            log_run('daily', run_log_anchor, started_at, len(daily_df), 'success')

        except Exception as e:
            logger.error(f"\n[FAIL] Failed to write to ClickHouse: {str(e)}")
            log_run('daily', run_log_anchor, started_at, 0, 'failed', str(e))
            print(f"\n[WARN]  Patterns saved to CSV but failed to write to ClickHouse")
            raise

    else:
        print("\n[WARN]  No daily patterns detected with current thresholds")
        log_run('daily', run_log_anchor, started_at, 0, 'success')
